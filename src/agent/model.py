...
class Agent:
  def __init__(self, w=0):
    ...
    self.dims = np.array([self.numInputs, 50, 25, 25, 15, self.numOutputs])
    if w == 0:
      self.weights = []
      for i in range(self.dims.size-1):
        self.weights.append(np.random.rand(self.dims[i], self.dims[i+1])*50 - 25) #*10 -5
    else:
  ...
  def mutate(self):
    ...
    # rw = np.random.rand() * 5 # 6-3
    rw = np.random.rand(self.weights[lay].shape[1])*50 - 25 # 6-3
    # self.weights[lay][row][col] += rw
    self.weights[lay][row] += rw

def reproduce(p1, p2, o1, o2):
  lay = np.random.randint(len(p1.dims)-1)
  lay2 = np.random.randint(len(p1.dims)-1)
